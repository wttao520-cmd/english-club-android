# -*- coding: utf-8 -*-
"""小学阶段核心词汇库：按主题分组，覆盖 501 个课标词。

来源：义务教育英语课程标准（二级）核心词表，参照人教/外研/译林主流教材。
每套主题作为一门内置课程写入数据库，以"单词模式"练习：
  - en   = 单词（练习目标）
  - zh   = 词性 + 词义
  - note = 音标 + 例句 + 译文 + 用法讲解（练习页提示区显示，音标行用 IPA 字体）
"""
...
VOCAB_COURSES = [
    {
        ...
        "desc": g["desc"],
        "items": [
            (w, zh, _note(ipa, ex, ex_zh, tip))
            for (w, ipa, zh, ex, ex_zh, tip) in g["items"]
        ],
    }
